...
function ProductCard({ product, wishlist, toggleWishlist, addToCart, darkMode }) {
  const dm = darkMode;
  ...
  return (
    ...
    onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-6px)"; e.currentTarget.style.boxShadow = "0 16px 48px rgba(30,27,23,0.13)"; }}
    onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "0 4px 20px rgba(30,27,23,0.06)"; }}
    >
      <button
      ...
      >
        <svg width="14" height="14" fill={isWishlisted ? C.primary : "none"} stroke={C.primary} strokeWidth="1.5" viewBox="0 0 24 24">
      ...
      <Link href={`/products/${product._id}`} style={{ display: "block" }}>
        ...
        <div style={{ padding: "1.1rem 1.25rem 0.75rem" }}>
          ...
          <Link href={`/artisans/${product.seller?._id}`} style={{ fontFamily: C.sans, fontSize: "0.72rem", color: C.primary, marginBottom: "0.75rem", display: "block", textDecoration: "none" }}>
...
function ProductSkeleton() {
  ...
  const cardBg = dm ? "#2a2218" : "#ffffff";
  return (
    <div style={{ background: cardBg, borderRadius: "1rem", overflow: "hidden", boxShadow: "0 4px 20px rgba(30,27,23,0.06)", height: "360px" }}>
      <div style={{ height: "220px", background: "linear-gradient(90deg, #ede7df 25%, #e8e1da 50%, #ede7df 75%)", backgroundSize: "200% 100%", animation: "shimmer 1.5s infinite" }} />
      <div style={{ padding: "1.1rem 1.25rem" }}>
        <div style={{ height: "8px", width: "60px", background: "linear-gradient(90deg, #ede7df 25%, #e8e1da 50%, #ede7df 75%)", backgroundSize: "200% 100%", animation: "shimmer 1.5s infinite", borderRadius: "4px", marginBottom: "0.5rem" }} />
        <div style={{ height: "16px", width: "80%", background: "linear-gradient(90deg, #ede7df 25%, #e8e1da 50%, #ede7df 75%)", backgroundSize: "200% 100%", animation: "shimmer 1.5s infinite", borderRadius: "4px", marginBottom: "0.5rem" }} />
      ...
      <div style={{ padding: "0 1.25rem 1.25rem" }}>
        <div style={{ height: "44px", width: "100%", background: "linear-gradient(90deg, #ede7df 25%, #e8e1da 50%, #ede7df 75%)", backgroundSize: "200% 100%", animation: "shimmer 1.5s infinite", borderRadius: "9999px" }} />
      </div>
    </div>
  );
}
