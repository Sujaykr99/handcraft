...
export default function EditProductPage() {
  ...
  const validate = () => {
    ...
    if (existingImages.length + images.length === 0) newErrors.images = "At least one product image is required";
    setErrors(newErrors);
  ...
  const handleImageUpload = (files) => {
    const newFiles = Array.from(files).slice(0, 5 - existingImages.length - images.length);
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
          ...
          <div style={{ background: cardBg, borderRadius: "1.25rem", padding: "2rem", border: `1px solid ${borderColor}` }}>
            ...
            <p style={{ fontFamily: C.sans, fontSize: "0.82rem", color: muted, marginBottom: "1.5rem" }}>
              Upload up to 5 images total. Drag to reorder (first image = main thumbnail).
            </p>
              ...
              {(existingImages.length + images.length) < 5 && (
                ...
                >
                  ...
                  <p style={{ fontFamily: C.sans, fontSize: "0.8rem", color: dragging ? C.primary : muted, textAlign: "center" }}>
                    {dragging ? "Drop images here" : `Add image ${existingImages.length + images.length + 1}`}
                  </p>
                  <input
                    id="image-upload"
                    type="file"
